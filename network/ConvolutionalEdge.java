...
public class ConvolutionalEdge extends Edge {
        ...
        for (int i = 0; i < weight.length; i++) {
            for (int j = 0; j < weight[i].length; j++) {
                for (int k = 0; k < weight[i][j].length; k++) {
                        weight[i][j][k] = (rand.nextGaussian() * range);
                }
            }
        }
    }
        ...
        for (int i = 0; i < weight.length; i++) {
            for (int j = 0; j < weight[0].length; j++) {
                for (int k = 0; k < weight[0][0].length; k++) {
                    weight[i][j][k] = (rand.nextDouble() * ((2 * range) - range));
                }
            }
        }
    }
    ...
    public void propagateForward(double[][][][] inputValues) {
        ...
        int batchSize = inputValues.length;

        for (int batch = 0; batch < batchSize; batch++) {
            for (int inputz = 0; inputz < this.outputNode.sizeZ; inputz++) {
                for (int inputy = 0; inputy < (this.outputNode.sizeY - (this.outputNode.padding*2)); inputy++) {
                    for (int inputx = 0; inputx < (this.outputNode.sizeX - (this.outputNode.padding*2)); inputx++) {
                        double sum = 0.0;
                        for (int filterz = 0; filterz < sizeZ; filterz++) {
                            for (int filtery = 0; filtery < sizeY; filtery++) {
                                for (int filterx = 0; filterx < sizeX; filterx++) {
                                    sum += inputValues[batch][inputz+filterz][inputy+filtery][inputx+filterx] * weight[filterz][filtery][filterx];
                                }
                            }
                        }
                        outputNode.inputValues[batch][inputz][inputy+outputNode.padding][inputx+outputNode.padding]+=sum;
                    }
                }
            }
        }
    ...
    public void propagateBackward(double[][][][] delta) {
        ...
        ConvolutionalNode node = outputNode;

        for(int  i =0; i < node.batchSize;i++) {
            for (int ZZ = 0; ZZ < node.sizeZ; ZZ++) {
                for (int YY = 0; YY < node.sizeY - (2*node.padding); YY++) {
                    for (int XX = 0; XX < node.sizeX - (2*node.padding); XX++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    inputNode.delta[i][ZZ+z][YY + y][XX + x] += weight[z][y][x] * delta[i][ZZ][YY][XX];
                                    weightDelta[z][y][x]  += inputNode.outputValues[i][z + ZZ][y + YY][x + XX] * delta[i][ZZ][YY][XX];
                                }
                            }
                        }
                    }
