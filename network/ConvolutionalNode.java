...
public class ConvolutionalNode {
    ...
    public void propagateForward(boolean training) {
        ...
        if (nodeType == NodeType.HIDDEN) {
            ...
            mu_b = sum / (batchSize * sizeZ * sizeY * sizeX);
            double var = 0;
            for (int i = 0; i < batchSize; i++) {
                for (int z = 0; z < sizeZ; z++) {
                    for (int y = 0; y < sizeY; y++) {
                        for (int x = 0; x < sizeX; x++) {
                            var += (inputValues[i][z][y][x] - mu_b) * (inputValues[i][z][y][x] - mu_b);

                        }
                    }
                }
            }
            sigma2_b = var / (batchSize * sizeZ * sizeY * sizeX);
            if (useBatchNormalization) {
                        ...
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    ...
                                    if (generator.nextDouble() > (1 - dropoutRate)) {
    ...
    private void Relu(double[][][][] inputs) {
            ...
            if(layer>0) {
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                double inputValue = inputs[i][z][y][x];
                                outputValues[i][z][y][x] = Math.max(inputValue+ bias[z][y][x], 0) ;
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                double inputValue = inputs[i][z][y][x];
                                outputValues[i][z][y][x] = Math.min(Math.max(inputValue+ bias[z][y][x], 0), 5) ;
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                double inputValue = inputs[i][z][y][x];
                                outputValues[i][z][y][x] = Math.min(Math.max(inputValue+ bias[z][y][x], 0), 5) + leak * Math.min(0, inputValue+ bias[z][y][x]) ;
                            }
                        }
                    }
                }
            ...
            if (activationType == ActivationType.NONE) {
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                double input = inputs[i][z][y][x] + bias[z][y][x];
                                if (input >= 0) {
                                    biasDelta[i][z][y][x] += delta[i][z][y][x];
                                } else {
                                    delta[i][z][y][x] = 0;
            ...
            if (useBatchNormalization) {
                    ...
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                ...
                                gammaDelta += xHat[i][z][y][x] * delta[i][z][y][x];
                                delta[i][z][y][x] *= gamma;
                            }
                        }
                    }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                sumInput += delta[i][z][y][x] * (inputValues[i][z][y][x] - mu_b);
                            }
                        }
                    }
                }
                double temp = sumInput / ((batchSize * sizeZ * sizeY * sizeX) * (Math.pow(sigma2_b + epsilon, (double) 3 / 2)));

                double[][][][] mu_k = new double[batchSize][sizeZ][sizeY][sizeX];
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                mu_k[i][z][y][x] = (delta[i][z][y][x] / Math.sqrt(sigma2_b + epsilon)) - (inputValues[i][z][y][x] - mu_b) * temp;
                            }
                        }
                    }
                }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                summation += mu_k[i][z][y][x];
                            }
                        }
                    }
                }
                double division = summation / (batchSize * sizeZ * sizeY * sizeX);
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                delta[i][z][y][x] = mu_k[i][z][y][x] - division;
                            }
                        }
                    }
                }
    ...
    public void initializeWeightsAndBiasKaiming(double initialBias, int fanIn) {
        ...
        WeightAndBiasInitializeHelper(initialBias);
        double[][][] weights = new double[sizeX][sizeY][sizeZ];
        for (int z = 0; z < weights.length; z++) {
            for (int y = 0; y < weights[0].length; y++) {
                for (int x = 0; x < weights[0][0].length; x++) {
                    weights[z][y][x] = generator.nextGaussian() / Math.sqrt(fanIn+1);
                }
            }
        }
    }

    private void WeightAndBiasInitializeHelper(double initialBias) {
        this.bias = new double[sizeZ][sizeY][sizeX];
        for (int z = 0; z < sizeZ; z++) {
            for (int y = 0; y < sizeY; y++) {
                for (int x = 0; x < sizeX; x++) {
                    this.bias[z][y][x] = initialBias;
                }
            }
        }
    }
    ...
    public void initializeWeightsAndBiasXavier(double initialBias, int fanIn, int fanOut) {
        ...
        WeightAndBiasInitializeHelper(initialBias);
        int totalFan = fanIn + fanOut;
        double range = Math.sqrt(6.0 / (totalFan)+1);

        double[][][] weights = new double[sizeX][sizeY][sizeZ];
        for (int z = 0; z < weights.length; z++) {
            for (int y = 0; y < weights[0].length; y++) {
                for (int x = 0; x < weights[0][0].length; x++) {
                    weights[z][y][x] = (generator.nextDouble() * 2.0 - 1.0) * range;
                }
            }
        }
